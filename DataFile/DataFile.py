...
if __name__ == '__main__':
    ...
    for item in filenameList:
            print item
            df = DataFile(item).getData("Location")
            df = df[['datetime', 'second', 'nanosecond', 'poseX', 'poseY', 'heading']]
            df.to_csv("e:\\data\\%02d.csv" % i)
            i += 1
            each[item] = df
            dfs.append(df)
    all = pd.concat(dfs)
    all.to_csv("e:\\data\\locb.csv")
